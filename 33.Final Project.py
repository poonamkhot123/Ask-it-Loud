...
5.Invert pixel value and then enchance the result for better output.ResourceWarning
6.Find Contours for specific colored object.object
7.find max area contour and draw it on live feed.
8. find Convexity detect for counting values and Apply Cosin method
9.Bind hand gestures with keyboard keys.KeyboardInterrupt
...
video = cv2.VideoCapture("sample.mp4")  # video file path
...
while True:
    ...
    try:
        ...
        epsilon = 0.0005 * cv2.arelength(cm,True)
        ...
        hull = cv2.convecHull(cm,returnPoints=False)
        ...
        if count_defects == 0:
            ...
            cv2.putText(frame,"Volume Down",(50,50),cv2.FONT_HERSHEY_SIMPLEX,2,(0,0,2))
